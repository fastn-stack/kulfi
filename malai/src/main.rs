        ...
        Some(Command::External(args)) => {
            ...
            if args.len() >= 1 {
                ...
                if args.len() >= 2 {
                    ...
                    // Use malai_server send_command for now (direct P2P, no daemon socket)
                    if let Err(e) = test_remote_command(machine, command, cmd_args).await {
...
/// Test remote command execution (simplified for MVP)
async fn test_remote_command(machine_address: &str, command: &str, args: Vec<String>) -> Result<(), eyre::Error> {
    use std::str::FromStr;
    ...
    println!("🎯 Target: {} in cluster {}", machine_alias, cluster_alias);
    
    // Find cluster and get our identity for this cluster
    let malai_home = if let Ok(home) = std::env::var("MALAI_HOME") {
        std::path::PathBuf::from(home)
    } else {
        dirs::data_dir().unwrap_or_default().join("malai")
    ...
    // Try identity.key (TODO: update to role-specific names)
    let identity_file = cluster_dir.join("identity.key");
    if !identity_file.exists() {
        return Err(eyre::eyre!("No identity found for cluster: {}", cluster_alias));
    }
    
    let key_content = std::fs::read_to_string(&identity_file)?;
    let our_identity = fastn_id52::SecretKey::from_str(&key_content.trim())?;
    
    // Get target machine ID52 from cluster config
    ...
    let target_id52 = if let Some(machine_section) = config.get("machine") {
        if let Some(machines) = machine_section.as_table() {
            ...
            return Err(eyre::eyre!("No machines section"));
        }
    } else {
        return Err(eyre::eyre!("No machine section in config"));
    };
    
    println!("🔑 Using identity: {}", our_identity.id52());
    println!("🎯 Target machine: {}", target_id52);
    
    // Self-command optimization: bypass P2P if targeting self
    if target_id52 == our_identity.id52() {
        println!("🔄 Self-command detected - executing locally (bypass P2P)");
        execute_self_command(&cluster_config_path, &our_identity.id52(), command, args).await
    } else {
        println!("📡 Remote command - sending via P2P");
        malai::send_command(our_identity, &target_id52, command, args).await
    }
}

/// Execute self-command locally with cluster.toml ACL validation
async fn execute_self_command(
    cluster_config_path: &std::path::Path,
    our_id52: &str, 
    command: &str,
    args: Vec<String>
) -> Result<(), eyre::Error> {
    println!("💻 Self-executing: {} {:?}", command, args);
    
    // Validate ACL using cluster.toml (since we're cluster manager)
    let config_content = std::fs::read_to_string(cluster_config_path)?;
    
    // Basic ACL check (MVP implementation)
    if !validate_self_acl(&config_content, our_id52) {
        return Err(eyre::eyre!("Access denied - self-command not permitted"));
    }
    
    println!("✅ ACL validated - executing command locally");
    ...
    match Command::new(command).args(&args).output().await {
        Ok(output) => {
            // Display output just like remote commands
            print!("{}", String::from_utf8_lossy(&output.stdout));
            eprint!("{}", String::from_utf8_lossy(&output.stderr));
            
            if output.status.success() {
                println!("✅ Self-command executed successfully");
            } else {
                println!("❌ Self-command failed with exit code: {}", 
                        output.status.code().unwrap_or(-1));
            }
            Ok(())
        }
        Err(e) => {
            Err(eyre::eyre!("Failed to execute self-command: {}", e))
        }
    }
}

/// Basic ACL validation for self-commands (MVP implementation)
fn validate_self_acl(config_content: &str, our_id52: &str) -> bool {
    // Simple validation - check if we're in the config with allow_from
    config_content.contains(&format!("id52 = \"{}\"", our_id52)) &&
    (config_content.contains("allow_from = \"*\"") || config_content.contains(our_id52))
}
